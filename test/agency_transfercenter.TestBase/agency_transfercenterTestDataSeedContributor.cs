...
public class agency_transfercenterTestDataSeedContributor : IDataSeedContributor, ITransientDependency
{
  ...
  public agency_transfercenterTestDataSeedContributor(
  ...
  private async Task CheckForAddingStation(int lineId)
  {
    if (await _stationRepository.CountAsync(x => x.LineId == lineId) > LineConst.LimitOfStation)
      throw new UserFriendlyException(AtcDomainErrorCodes.LimitOfStationError);
  }
  #endregion
